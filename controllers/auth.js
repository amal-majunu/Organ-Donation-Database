        ...
        db.query('SELECT * FROM donor where Username=?',[username], async (err,resuse)=>{
            if(err){
                ...
                db.query('SELECT * FROM donor where email=?', [email], async (err,results) => {
                    ...
                    let user = {
                                    Name : name, Hospital : req.body.hospital, Username : username,Password : hashedPassword,
                                    Personal_Doctor : req.body.doctor, Gender : req.body.gender,Email : req.body.email,
                                    Address : req.body.address, State : req.body.state, City : req.body.city,
                                    Contact : req.body.contact, DOB : req.body.dob, Ailments : req.body.disease
                                }
                    db.query('INSERT INTO nextofkin SET ?', {Name : req.body.nameofkin, Contact : req.body.kincontact, Username : username}, async (err,result)=>{
                        if(err){
                            console.log(err);
                        }else{
                            console.log(result);
...
exports.organavail = async (req,res) => {
    try {
        ...
        db.query('SELECT S.Organ_name,S.City,S.Address, S.Contact_Number,S.Hospital from (SELECT * from organs NATURAL JOIN hospitaldetails) as S where S.Organ_name = ? AND S.City = ?',
        [organ,city], async (err,results)=>{
            if(err){
                res.sendStatus(404);
            }else{
                if(results.length > 0){
